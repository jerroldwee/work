...
set :keep_releases, 5
set :application, "Figo Eyewear"
set :repository,  "git@gitlab.projectwebby.com:rubifytechnology/figo_web.git"
set :scm, :git
...
namespace :deploy do
  ...
  task :restart, :roles => :web do
    if remote_file_exists?("#{shared_path}/pids/figo_app.pid")
      run "kill -s QUIT `cat #{shared_path}/pids/figo_app.pid`"
    end
